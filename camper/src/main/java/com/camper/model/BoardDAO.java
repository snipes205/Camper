...
public class BoardDAO {
	...
	public BoardListTO boardList( BoardListTO listTO ) {
		...
		try {
			...
			String sql = 
			"select pseq, title, nick, type, date_format( wdate, '%y-%m-%d' ) wdate from p_table order by pseq desc";
			pstmt = conn.prepareStatement( sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY );
	...
	// 글쓰기 ( board_write.jsp )
	public BoardTO boardWrite( BoardTO to ) {
	...
	public int boardMoidfyOk(BoardTO to) {
		...
		try{
			...
			String sql = "update p_table set title=?, content=? where pseq=? and pwd=?";
			pstmt = conn.prepareStatement( sql );
			pstmt.setString( 1, to.getTitle() );
